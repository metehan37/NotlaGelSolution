    ...
    public class NoteController : Controller
    {
        ...
        public ActionResult GetLiked(int[] ids)
        {
            if(CurrentSession.user != null)
            {
                ...
                if (ids!=null)
                {
                    likedNoteIds = likedManager.List(x => x.LikedUser.Id == userid && ids.Contains(x.Note.Id)).Select(x => x.Note.Id).ToList();
                }
                else
                {
                    likedNoteIds = likedManager.List(x => x.LikedUser.Id == userid).Select(x => x.Note.Id).ToList();
                }
